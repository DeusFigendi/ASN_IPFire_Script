...
#######################################################################
  revision="asn_ipfire.sh v0.6.3_beta"             # do not comment out
# Last updated: August 30 2017 by maloe
...
# ASN sources, function must return a list of ASNs
	gather_ASN0() 	# Get ASN from local file
				{	
					if [[ -f $local_asn_file ]]; then 
						cname=`echo $1 | sed 's/%20/ /g'` 								#replace %20 with space
						cat $local_asn_file | grep -i "$cname" | grep -Eo 'AS[0-9]+'
					fi; 
				}
	gather_ASN1() 	# Get ASN from ultratools.org
				{	
					cname=`echo $1 | sed 's/+/%2B/g'`  									#replace "+" with %2B
					wget -q -O - --https-only "http://www.ultratools.com/tools/asnInfoResult?domainName=$cname" | grep -Eo 'AS[0-9]+' | uniq; 
				}
	gather_ASN2() 	# Get ASN from cidr-report.org
				{	
					cname=`echo $1 | sed 's/%20/ /g'` 								#replace %20 with space
					wget -q -O - --https-only "http://www.cidr-report.org/as2.0/autnums.html" | grep -i "$cname" | grep -Eo 'AS[0-9]+'
				}
	gather_ASN3() 	# Get ASN from bgplookingglass.com (part3)
				{ 
					cname=`echo $1 | sed 's/%20/ /g'` 								#replace %20 with space
					wget -q -O - --https-only "http://www.bgplookingglass.com/list-of-autonomous-system-numbers" | sed 's/<br /\n/g' | grep -i "$cname" | grep -Eo 'AS[0-9]+'			# Get ASN from bgplookingglass.com (part1)
					wget -q -O - --https-only "http://www.bgplookingglass.com/list-of-autonomous-system-numbers-2" | sed 's/<br /\n/g' | grep -i "$cname" | grep -Eo 'AS[0-9]+'		# Get ASN from bgplookingglass.com (part2)
					wget -q -O - --https-only "http://www.bgplookingglass.com/4-byte-asn-names-list" | sed 's/<br /\n/g' | grep -i "$cname" | grep -Eo 'AS[0-9]+'
				}
...
# Network sources, must return a list of CIDR networks
	...
	gather_NET1() { wget -q -O - --https-only "https://stat.ripe.net/data/announced-prefixes/data.json?preferred_version=1.1&resource=$1" | grep -Eo '([0-9.]+){4}/[0-9]+' | sort -u ; }	# Get networks from stat.ripe.net, rough sorting
	gather_NET2() { wget -q -O - --https-only "https://ipinfo.io/$1" | grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}' | sort -u ; }														# Get networks from ipinfo.io, rough sorting
...
# Main procedures
	addNetworks() {
		...
		for company in ${company_array[@]}; do
			...
			declare asn_array=()
			declare asn_list=()
			prnt_company=`echo $company | sed 's/%20/ /g; s/%2B/+/g'`										# Company name with space and "+"
	...
	case $1 in
		-f | --file) {
			if [[ -f $2 ]]; then													# File exist
				company_array_from_file=(`sed 's/[ \t]*,\+[ \t]*/,/g; s/[\/]//g; /,/s/[ ]/%20/g; s/,/ /g' <<< cat $2`)	# Substitute space,comma,slash
				shift
		...
		-a|--add | -r|--remove | --asn | --network | --network_raw | --iptable | --afwall) {
			...
			if [[ $2 && "${2:0:1}" != "-" ]]; then 									# No company names given
				company_array_from_arg=(`sed 's/[ \t]*,\+[ \t]*/,/g; s/[\/]//g; /,/s/[ ]/%20/g; s/,/ /g' <<< $2`)	# Substitute space,comma,slash
				shift
			fi
		};;
